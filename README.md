...
| 스킬 | 설명 |
|------|------|
| `/law-check` | 국가법령정보 MCP 기반 법령/윤리 실시간 검토, 의무신고 판단, 시설기준/별표 조회 |
| `/law-research` | 복지 법령 심층 조사 보고서 생성 (법률-시행령-시행규칙-조례-판례-해석례 종합) |
...
/law-check
→ "아동학대 의심 상황에서 신고의무 확인" → 국가법령정보 MCP로 실시간 법령 조회 → 신고 절차 안내

/law-research
→ "장애인 활동지원 서비스 관련 법령 종합 조사" → 법률~조례~판례 체계적 수집 → 조사 보고서 생성
```
